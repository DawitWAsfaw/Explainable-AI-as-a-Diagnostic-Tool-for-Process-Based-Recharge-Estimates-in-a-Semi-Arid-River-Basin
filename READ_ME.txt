...
Download climatic data from Google Earth Engine -using "gee_download.py". The shapefiles in "gis" folder contains a subfolder "gee_download_shpfiles" 
...
Create hydrological factors maps using ArcGIS Pro using "Elevation" data downloaded from gee (1 -3) and larb_rivers.shp (4 -5)
 1. Topographics wetness index (TWI),
 2. Topographics roughness index (TRI, 
...
step two: Click the "Open Data Downloader Tool" - down arrow symbol and go " Tool" menu on the right pane, under "Data Download", "Method:" select "Shapefile" and and click on "Choose File" icon
browse to the shapefile folder under "GIS->shapefiles->lulc_download_shapefile->larb_epsg_3857.zip" and under options  "GeoTIFF". Once your shapefile is uploaded and is visible on the CONUS map , 
...
Analyse Annual LULC Percentage per HUC12 polygon scale
step one: Rerpoject the LULC .tiff files and save the reproject file under "500m_scale/tiff/" folder.
...
Analyse Annual LULC Percentage per at 500m grid scale  
...
2. The 500m grid shapefile is provide under "GIS->shapefiles->500m_grid->huc12_basins.shp"
...
step two: To find the area symbol for the soil data boundary go to "https://casoilresource.lawr.ucdavis.edu/gmap/" (Accessed 11/7/2025 at 4:39pm) and 
			identify the codes to add to the SSURGO Bulk Downloader Toolbox Search by Areasymon dropdown menu. 
step three: Go to https://www.nrcs.usda.gov/sites/default/files/2024-11/SSURGO-Portal-Quick-Start-Guide.pdf  (Accessed 11/7/2025 at 4:42pm)
			and folllow the instruction to install SSURGO Portal. 
			...
			include "January" as Begining Month and " December" as Ending month. 
...
Save the downloaded data under " grid_500m" folder under subfolders correponsing to each variables
 1. "climate_data" 
 2. "soil_data" and 
...
Calculate zonal statistics - for all LULC Classes in addition to Majority classe and unique classes
huc12_scale:
